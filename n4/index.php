...
//Подразумевается, что этот массив $values затем используется для предварительного заполнения полей формы на веб-странице, чтобы пользователь видел свои предыдущие вводы, 
//даже если он покинул страницу и вернулся обратно. Это удобно для сохранения и восстановления введенных данных.
  $values = array();
...
//Итак, данный код обрабатывает выбранные суперспособности из формы. Он устанавливает cookie для активированных суперспособностей и обновляет состояние массива $apw. 
//Если суперспособность была деактивирована (значение в массиве $apw равно 0), соответствующее cookie будет удалено. Это позволяет хранить информацию о состоянии суперспособностей на устройстве пользователя.
  else {
    $powrs=$_POST['super'];
    $apw=array(
      "inv_v"=>0,
